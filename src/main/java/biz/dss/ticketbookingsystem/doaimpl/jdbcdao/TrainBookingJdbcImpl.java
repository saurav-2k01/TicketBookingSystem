...
public class TrainBookingJdbcImpl implements TrainBookingDao {
    ...
    public List<TrainBooking> getTrainBookings() throws SQLException{
        ResultSet resultSet;
        try (PreparedStatement preparedStatement = connection.prepareStatement(SqlQueries.GET_ALL_TRAIN_BOOKING)) {
    ...
    public Optional<TrainBooking> addTrainBooking(TrainBooking trainBooking) throws SQLException {
        try (PreparedStatement preparedStatement = connection.prepareStatement(SqlQueries.ADD_TRAIN_BOOKING)) {
            ...
            return rowAffected>0?Optional.of(trainBooking):Optional.empty();
        }
    }

    public Optional<TrainBooking> updateTrainBooking(TrainBooking trainBooking) throws SQLException {
        try (PreparedStatement preparedStatement = connection.prepareStatement(SqlQueries.UPDATE_TRAIN_BOOKING)) {
    ...
    public Optional<TrainBooking> getTrainBooking(TrainBooking trainBooking) throws SQLException{
        try(PreparedStatement preparedStatement =  connection.prepareStatement(SqlQueries.GET_TRAIN_BOOKING)){
